...
<c:if test="${scope==0 || scope==1}">
	...
	<tr>
		...
		<td class="form_input menuTool">
			<a class="look" target="_blank" href="${ctx}/cmsbuild/preview.chtml?siteid=${po.siteid}&categoryid=${po.id}&&isedit=true">查看栏目内容</a><c:if test="${enablemobile}">
			<a class="look" target="_blank" href="${ctx}/cmsbuild/preview.chtml?siteid=${po.siteid}&categoryid=${po.id}&mobile=true&isedit=true">查看移动版栏目内容</a></c:if>
		</td>
	</tr>
	<tr>
