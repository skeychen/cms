...
<table border="0" cellspacing="1" cellpadding="0" class="listTable">
	...
	<tr>
		...
		<c:if test="${po.scope!=2}">
			<a class="look" target="_blank" href="${ctx}/cmsbuild/preview.chtml?siteid=${po.siteid}&categoryid=${po.categoryid}&pageid=${po.id}&isedit=true">查看内容</a><c:if test="${enablemobile}">
			<a class="look" target="_blank" href="${ctx}/cmsbuild/preview.chtml?siteid=${po.siteid}&categoryid=${po.categoryid}&pageid=${po.id}&mobile=true&isedit=true">查看移动版内容</a></c:if>
		</c:if>
		</td>
	</tr>
	<tr>
