...
<c:if test="${category.scope==0}">
	<c:if test="${fn:length(category.ptype) > 0}"><tr>
		...
		<td class="form_input">
			<c:if test="${fn:length(category.ptypeitemList) == 0}"><input type="text" name="ptype" maxlength="100" style="width:300px;" value="${fn:escapeXml(po.ptype)}" /></c:if>
			<c:if test="${fn:length(category.ptypeitemList) > 0}"><select name="ptype" v="${fn:escapeXml(po.ptype)}">
	...
	<tr${fn:length(category.pname) > 0 ? "" : " style='display:none;'"}>
		...
		<td class="form_input"><input type="text" class="WebDate" format="${fn:escapeXml(category.pformat)}" fn="mybegin" value="${po.pbegin>0?po.pbegin:''}" />
			至
			<input type="text" class="WebDate" format="${fn:escapeXml(category.pformat)}" fn="myend" value="${po.pend>0?po.pend:''}" />
		</td>
	</tr>
</c:if>
	<tr>
...
<input type="hidden" id="pbegin" name="pbegin" value="${po.pbegin}" />
<input type="hidden" id="pend" name="pend" value="${po.pend}" />
</form>
</body>
</html>
